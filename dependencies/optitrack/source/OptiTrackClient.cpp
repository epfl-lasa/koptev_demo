...
namespace optitrack
{
  ...
  bool OptiTrackClient::testConnection()
  {
    ...
    int ret = client_->SendMessageAndWait("FrameRate", 20, 200, &response, &nBytes);
    ...
    printf("[testConnection] TEST RATE : %f\n", response);

    ret = client_->SendMessageAndWait("CurrentMode", 20, 200, &response, &nBytes);
    printf("[testConnection] TEST CurrentMode : %i \n", response);
  ...
  {
    printf("[Optitrack client] - Callback Set");
    callback_ = callback;
  }
